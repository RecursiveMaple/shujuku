...
-->
      ...
      if (uidsToDelete.length > 0) {
        ...
        if (settings_ACU.knownCustomEntryNames) {
          if (settings_ACU.dataIsolationEnabled) {
            ...
            settings_ACU.knownCustomEntryNames = settings_ACU.knownCustomEntryNames.filter((n) =>
              n.startsWith("ACU-[")
            ); // 只保留隔离的
          }
          saveSettings_ACU();
  ...
  async function purgeSheetKeysFromChatHistoryHard_ACU(sheetKeysToPurge) {
    ...
    const hasAnySheetKey = (obj) =>
      obj && typeof obj === "object" && Object.keys(obj).some((k) => k.startsWith("sheet_"));
    const safeClone = (obj) => {
      try {
        ...
        try {
          ...
          if (toFix.length > 0) {
            await TavernHelper_API_ACU.setLorebookEntries(
              primaryLorebookName,
              toFix.map((e) =>
                applySystemDepthInjection_ACU({ uid: e.uid, order: sharedSummaryDataOrder }, SUMMARY_FIXED_SYSTEM_DEPTH)
              )
            );
          }
        } catch (e) {
      ...
      try {
        ...
        if (isDatabaseEmpty) {
          ...
          const memoryEndOld = entries.find(
            (e) =>
              e.comment === isoPrefix + (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-MemoryEnd` : "TavernDB-ACU-MemoryEnd")
          );
        ...
        if (summaryEntriesToReorder.length > 0) {
          await TavernHelper_API_ACU.setLorebookEntries(
            primaryLorebookName,
            summaryEntriesToReorder.map((e) =>
              applySystemDepthInjection_ACU({ uid: e.uid, order: summaryDataOrder }, SUMMARY_FIXED_SYSTEM_DEPTH)
            )
          );
        }
      ...
      const personsHeaderEntryData = applySystemDepthInjection_ACU(
        {
          ...
          keys: [
            isoPrefix +
              (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-PersonsHeader-Key` : "TavernDB-ACU-PersonsHeader-Key"),
          ],
          enabled: true,
          type: "constant",
          order: null,
      ...
      if (allCreates.length > 0) {
        ...
        try {
          ...
          if (header?.uid)
            updates.push(
              applySystemDepthInjection_ACU(
                { uid: header.uid, order: personsOrderBlockBase },
                PERSONS_FIXED_SYSTEM_DEPTH
              )
            );
          rows.forEach((e) => {
            if (e?.uid)
              updates.push(
                applySystemDepthInjection_ACU(
                  { uid: e.uid, order: personsOrderBlockBase + 1 },
                  PERSONS_FIXED_SYSTEM_DEPTH
                )
              );
          });
...
insertRow(1, ["时间2", "大纲事件2...", "关键词"]);
  ...
  function parseAndApplyTableEdits_ACU(aiResponse, updateMode = "standard", allowedSheetKeys = null) {
    // updateMode: 'standard' 表示更新标准表，'summary' 表示更新总结表和总体大纲
    if (!currentJsonTableData_ACU) {
    ...
    let appliedEdits = 0;
    const editCountsByKey = {}; // Map<sheetKey, count>
    const allowedSheetKeysSet =
      Array.isArray(allowedSheetKeys) && allowedSheetKeys.length ? new Set(allowedSheetKeys) : null;

    const sheetKeysSorted = Object.keys(currentJsonTableData_ACU)
      .filter((k) => k.startsWith("sheet_"))
      .sort((a, b) => {
        const numA = parseInt(a.replace("sheet_", ""), 10);
        const numB = parseInt(b.replace("sheet_", ""), 10);
    ...
    const sheetInfos = sheetKeysSorted.map((key) => ({
      key,
      table: currentJsonTableData_ACU[key],
      name: currentJsonTableData_ACU[key]?.name || "",
    }));
      ...
      try {
        ...
        switch (command) {
          case "insertRow": {
            ...
            const tableInfo = sheetInfos[tableIndex];
            const table = tableInfo ? tableInfo.table : null;
            const sheetKey = tableInfo ? tableInfo.key : null;
            if (!table || !table.name) {
              logWarn_ACU(`Table at index ${tableIndex} not found or has no name. Skipping insertRow.`);
              break;
            }
            if (allowedSheetKeysSet && (!sheetKey || !allowedSheetKeysSet.has(sheetKey))) {
              logDebug_ACU(`[屏蔽] 当前更新范围排除了表格 ${table.name || sheetKey}，跳过 insertRow.`);
              break;
            }
            ...
            if (table && table.content && typeof data === "object") {
              ...
              appliedEdits++;
              if (sheetKey) {
                editCountsByKey[sheetKey] = (editCountsByKey[sheetKey] || 0) + 1;
              }
            }
            break;
          }
          case "deleteRow": {
            ...
            if (allowedSheetKeysSet && (!sheetKey || !allowedSheetKeysSet.has(sheetKey))) {
              logDebug_ACU(`[屏蔽] 当前更新范围排除了表格 ${table.name || sheetKey}，跳过 deleteRow.`);
              break;
            }
            // [新增] 根据更新模式和表格名称屏蔽不相关的表格操作
            // [修复] 统一更新模式（'full'）允许所有操作，不阻止任何表
          ...
          case "updateRow": {
            ...
            if (allowedSheetKeysSet && (!sheetKey || !allowedSheetKeysSet.has(sheetKey))) {
              logDebug_ACU(`[屏蔽] 当前更新范围排除了表格 ${table.name || sheetKey}，跳过 updateRow.`);
              break;
            }
            // [新增] 根据更新模式和表格名称屏蔽不相关的表格操作
            // [修复] 统一更新模式（'full'）允许所有操作，不阻止任何表
    ...
    Object.keys(editCountsByKey).forEach((sheetKey) => {
      const tableObj = currentJsonTableData_ACU[sheetKey];
      if (tableObj) {
        if (!tableObj._lastUpdateStats) {
          tableObj._lastUpdateStats = {};
        }
        tableObj._lastUpdateStats.changes = editCountsByKey[sheetKey];
      }
    });

    // [新增] 收集所有被修改的表格 key
    const modifiedSheetKeys = Object.keys(editCountsByKey).filter((key) => editCountsByKey[key] > 0);
  ...
  async function checkAndTriggerAutoMergeSummary_ACU() {
    ...
    const outlineKey = Object.keys(currentJsonTableData_ACU).find(
      (k) => currentJsonTableData_ACU[k].name === "总体大纲"
    );
  ...
  async function proceedWithCardUpdate_ACU(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        const parseResult = parseAndApplyTableEdits_ACU(aiResponse, updateMode, targetSheetKeys);
